...
export default function EmployeesPage() {
  ...
  const [form, setForm] = useState({ id: '', name: '', username: '', password: '', role: 'Cashier', branchId: '' })
                  ...
                  <tr key={u.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.2s', ...({ ':hover': { background: 'rgba(255,255,255,0.02)' } } as any) }}>
                    ...
                    <td style={{ padding: '1.25rem 1.5rem' }}>
                       <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', padding: '0.35rem 0.8rem', borderRadius: 50, fontSize: '0.75rem', fontWeight: 700, 
                         ...
                         <Shield size={12} />
                         {u.role === 'SuperAdmin' ? 'مالك (SuperAdmin)' : u.role === 'Admin' ? 'مدير نظام (Admin)' : u.role === 'Manager' ? 'مدير فرع (Manager)' : 'كاشير (Cashier)'}
                       </span>
                    </td>
                    <td style={{ padding: '1.25rem 1.5rem' }}>
                       {u.role === 'Cashier' ? (
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <select value={form.role} onChange={e => setForm({...form, role: e.target.value as any, branchId: e.target.value !== 'Cashier' ? '' : form.branchId})} {...inpProps}>
                       {form.role === 'SuperAdmin' && <option value="SuperAdmin">مالك النظام (SuperAdmin)</option>}
                       <option value="Admin">مدير نظام (Admin)</option>
                       <option value="Manager">مدير مالي/فرع (Manager)</option>
